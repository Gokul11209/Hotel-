...
class HotelFolio(models.Model):
    ...
    def action_cancel(self):
        ...
        return {
            ...
            'name': 'Hotel Management Proformo Order Cancel Remarks',
            'res_model': 'folio.order.cancel',
            'view_type': 'form',
            'view_mode': 'form',
            'res_id': view_id.id,
            'view_id': self.env.ref('hotel.landry_folio_cancel_remarks_wizard', False).id,
            'target': 'new',
        }
        # for rec in self:
        #     if not rec.order_id:
